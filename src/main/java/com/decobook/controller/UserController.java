...
public class UserController {
	...
	//로그인
	@PostMapping("/login")
	@Operation(summary = "로그인", description = "유저 아이디와 비밀번호를 이용하여 액세스 토큰을 반환합니다.")
	public ResponseEntity<Map<String, Object>> login(
		...
		if (user_id != null && password != null && list.size() > 0 && list.get(0).getPassword().equals(password)) {
			res.put("access-token", jwtUtil.createToken("id", user_id));
			res.put("message", "로그인 성공");
			// res.put("user_name", list.get(0).getUser_name());
			status = HttpStatus.OK;
		} else {
			res.put("message", "로그인 실패");
...
//	}
	...
	public ResponseEntity<?> registUser(@RequestBody User user){ //@ModelAttribute?
		...
		Integer res = uService.registUser(user);
	...
	public ResponseEntity<?> updateUser(User user){
		...
		Integer res = uService.updateUser(user);
